...
export class DiscordStrategy extends PassportStrategy(Strategy, 'discord') {
  ...
  constructor(
    ...
    this.stateEncryptionKey = Buffer.from(
      environmentService.getStateEncryptionKey(),
      'hex',
    );
    this.stateEncryptionIV = Buffer.from(
      environmentService.getStateEncryptionIV(),
      'hex',
    );
  }
  ...
  async validate(
    ...
    try {
      ...
      this.logger.debug(`Found workspace: ${JSON.stringify(workspace)}`);

      if (!workspace) {
        this.logger.error(
          `Workspace not found for ID: ${stateData.workspaceId}`,
        );
      ...
      if (!workspace.discordGuildId) {
        this.logger.error(
          `Discord guild ID not configured for workspace: ${stateData.workspaceId}`,
        );
        throw new UnauthorizedException(
          'Discord guild not configured for workspace',
        );
      }

      const workspaceGuildId = workspace.discordGuildId;
      this.logger.debug(
        `Validating guild membership for workspace ${stateData.workspaceId} and guild ${workspaceGuildId}`,
      );
